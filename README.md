...
- **Warp gates & supercruise** — neighbor-linked warp gates; fly into the aperture to jump, emerge from the paired gate. Supercruise to waypoints with standoff arrival. **System Scan (B)** for probe scanning of Spatial Anomalies (alien incursions, datacores) that spawn in **open system space** (clear of planets, moons, stations, and the star), not only near bodies.
...
| **J** | Missions tracker |
| **F1** | Character sheet |
| **Esc** | Open pause (resume from the menu; pause dismisses open panels) |
| **F5** | Quick save |
